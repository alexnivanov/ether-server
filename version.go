...
// Нужен он не сам по себе — а чтобы снимать способы работы, которые
// поддерживают только старые сборки: сборка ниже min получает блокирующий экран
// и не остаётся с молча отломанным путём.
//
...
const softDelay = 3 * 24 * time.Hour
