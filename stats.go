...
//
// Расписание — суббота, 7:00 по времени сервера. Час выбран так, чтобы сводка
// приходила в 10:00 по Москве: прод живёт в UTC, а читаем мы её в MSK (UTC+3).
// Зависимость неявная — если у сервера поменять зону, сводка уедет на три часа,
// и править придётся здесь.
//
// Минуты у прихода свои: проверка идёт РАЗ В ЧАС от старта процесса, а не сном
// до нужной минуты, и это принципиально: сервер перезапускается при каждом
...
// последняя суббота statsHour:00 включительно. Время местное — у сервера его
// собственная зона (см. statsHour про то, почему час именно такой).
func lastScheduled(now time.Time) time.Time {
	...
	if due.After(now) {
		// сегодня суббота, но час расписания ещё не наступил — отчитываемся за прошлую
...
func formatWeeklyStats(st *WeeklyStats, from, to time.Time) string {
	...
	fmt.Fprintf(&b, "\n🔗 Переходов по ссылке: %d", st.Accesses)
	// Отфильтрованное называем вслух: молча уменьшившийся счётчик читался бы как
	// «людей стало меньше», а это всего лишь ушли боты.
	if st.Previews > 0 {
...
// previewUAMarkers — по каким словам в User-Agent заход считается не человеком.
// Сборщики превью дёргают ссылку сами: человек только вставил её в чат, а
// Telegram, WhatsApp и Facebook уже сходили за картинкой — иногда по разу на
// каждого участника беседы. В app_access такие строки пишутся наравне с людьми
// (таблица хранит сырьё, и разбирать его задним числом можно, только ничего не
// выкинув), а вот в сводке они врут: задирают счётчик переходов, забивают
// построчный список и приписывают приглашающему заходы, которых не было.
//
// Опознаются по имени в User-Agent, а не по platform=unknown: мобильный
// Googlebot представляется Android’ом, и разбор UA считает его телефоном.
// Маркеры в нижнем регистре — сравнение регистронезависимое и в Go, и в SQL
// (LIKE в SQLite по умолчанию такой для ascii). Список заведомо неполный: чужой
// бот, которого здесь нет, попадёт в сводку человеком, и это лучше обратной
// ошибки — вычеркнуть живого по слишком широкому слову.
var previewUAMarkers = []string{
	"bot", // TelegramBot, Googlebot, Slackbot, Twitterbot, …
	"crawler",
	"spider",
	"whatsapp",
	"facebookexternalhit",
...
//
// Названия для превью и краулеров остаются, хотя в список сводки они уже не
// доходят (см. previewUAMarkers): список маркеров неполный, и бот, которого в
// нём нет, должен опознаваться глазами, а не притворяться человеком.
