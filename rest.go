...
// и нужно.
//
// Превью ссылок в мессенджерах (Telegram, WhatsApp) сами дёргают URL, и такие
// заходы тоже попадают в таблицу — намеренно: здесь сырьё, а из недельной сводки
// они отфильтровываются по User-Agent на чтении (см. previewUAMarkers в
// stats.go). Единственный заход, который в таблицу НЕ попадает, — наша
// собственная проверка деплоя (см. srcDeployCheck).
