...
func TestChannelsKnownLocations(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// Двойной сторож фильтра кандидатов (isUnit): в цепочке этой точки
			// лежат И числовая адресная сетка (place/quarter «5», «14», …), И
			// пешеходная зона «Красная площадь» (highway/pedestrian), И
			// историческая граница «Китай-город» (boundary/historic). Ни одно из
			// трёх каналом быть не может, а по рангу все они специфичнее
			// Тверского района — без фильтра они вытесняли его ИЗ НАБОРА.
			//
			// Квартала здесь поэтому нет вовсе: под Тверским районом настоящих
			// единиц не осталось, а Квартал по модели опционален.
			name: "Россия — Красная площадь: посторонние объекты не вытесняют настоящий район",
			...
			wantAbsent: []string{"region", "quarter"},
		},
		{
		...
		{
			// Город здесь пуст, и это НЕ наша логика: узел place/town «Monaco»
			// переехал в цепочке с ранга городской полосы (10–16) на 18, то есть
			// в подгородскую. Проверено 05.09.2026; прежде фикстура ждала
			// node/1790048269 в Городе. Пинить такое бессмысленно, пока источник
			// — публичный Nominatim: это ровно та нестабильность, ради которой
			// заведён план «Свой Nominatim» в ether-meta/PLANS.md.
