...
//
// ПРИВАТНОСТЬ: в сообщениях ошибок есть `user_id` — внутренний идентификатор
// пользователя (не id у провайдера входа: тот наружу не уходит вовсе).
...
func captureToSentry(r slog.Record, base []slog.Attr) {
	...
	// одна проблема остаётся одной записью, сколько бы разных user_id в неё ни
	// попало.
