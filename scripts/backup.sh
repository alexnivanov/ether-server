...
if [[ -n "${BACKUP_REMOTE:-}" ]]; then
	if [[ -z "${AGE_RECIPIENT:-}" ]]; then
		...
		# --ignore-times: без него rclone молча пропускает перезалив «latest».
		# Менялся файл или нет, он решает по размеру и времени, а время на WebDAV
		# Яндекса выставить нельзя — остаётся размер. Шифрованный tar конфига
		# весит одни и те же ~20 КБ изо дня в день, поэтому
		# ether-config-latest.tar.age застывал на самой первой версии: при
		# восстановлении сервера 16.09.2026 «последний» конфиг оказался месячной
		# давности, без блока client_versions. База спаслась только тем, что её
		# размер плавает, — сутки без новых сообщений дали бы ровно то же.
		# Перезаливать десятки килобайт каждую ночь дешевле, чем однажды
		# обнаружить, что копия «последняя» лишь по имени.
		put() { rclone --retries 3 -q --ignore-times copyto "$1" "${BACKUP_REMOTE%/}/$2"; }
